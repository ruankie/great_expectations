...
{IDDict(domain_kwargs).to_id()}"""
    ...
    def _build_selectable_from_batch_spec(
        ...
        subselectable: Selectable = self._subselectable(batch_spec)
        ...
        return sa.select("*").select_from(subselectable).where(split_clause)
    ...
    def _subselectable(self, batch_spec: BatchSpec) -> Selectable:
        ...
        if table_name:
            ...
            # Query is a valid SELECT query that begins with r"select\w"
            selectable = sa.select(sa.text(query[6:].lstrip()))
    ...
    def get_batch_data_and_markers(
        ...
        has_query = 1 if batch_spec.get("query") else 0
        has_table_name = 1 if batch_spec.get("table_name") else 0
        if has_query + has_table_name != 1:
            raise InvalidBatchSpecError(
